...
void i2c_setup(void) {
    ...
    i2c_peripheral_disable(I2C2); // Desactivar I2C2 antes de configurar
    rcc_periph_reset_pulse(RST_I2C2);

    i2c_set_standard_mode(I2C2); // Configurar modo estándar
    i2c_set_clock_frequency(I2C2, I2C_CR2_FREQ_36MHZ); // Configurar frecuencia del reloj a 36 MHz
    i2c_set_trise(I2C2, 36); // Configurar tiempo de subida
    i2c_set_dutycycle(I2C2, I2C_CCR_DUTY_DIV2); // Configurar ciclo de trabajo
    i2c_set_ccr(I2C2, 180); // Configurar CCR para 100 kHz
    i2c_peripheral_enable(I2C2); // Habilitar I2C2
...
bool i2c_start(uint8_t addr, bool read) {
    ...
    i2c_send_7bit_address(I2C2, addr, read ? I2C_READ : I2C_WRITE);
    ...
    while (!(I2C_SR1(I2C2) & I2C_SR1_ADDR)) {
        ...
        if (I2C_SR1(I2C2) & I2C_SR1_AF) {
            I2C_SR1(I2C2) &= ~I2C_SR1_AF; // Limpiar bandera de fallo de ACK
            print_uart("Error al establecer la comunicacion\n\r");
            i2c_send_stop(I2C2); // Detener comunicación
